...
var fields_notUse = ["OBJECTID", "ACTIVE", "POSITION_SRC", "CREATED_USER", "CREATED_DATE", "LAST_EDITED_USER", "LAST_EDITED_DATE", "SHAPE", "SHAPE.STAREA()", "SHAPE.STLENGTH", "SHAPE.STLength()", "SHAPE.STLENGTH()", "SORT_BY", "FRONT_IMG_URL", "REAR_IMG_URL", "GLOBALID"];
  ...
  parser, SpatialReference, GeometryService, esriConfig, Graphic, SimpleMarkerSymbol, SimpleFillSymbol, Color, dom, domAttr, DropDownButton, Menu, DropDownMenu, MenuItem, Dialog, registry, domConstruct, ready
	...
	function init()
	{
		...
		arcgisUtils.createMap(webmapId, "map").then(function (response) {
			...
			url_gravesite_poly = url_service +"/" + layer_gravesitePolygon_index;
			...
			// 
			//dijit.registry.byId("toEditor_btn").on("click", function() {
			//	toEditor_btn();
			//});
					...
					if(item == "features"){
						if (value.length > 0){
							for(var j = 0; j < value.length; j++) {
								...
								addPointToMap(geom, attributes, "point");
							}
						}
						else{
							$.getJSON(url_poly, function(result){
								var items = [];
								$.each(result, function(item, value){
									if(item == "features"){
										if (value.length > 0){
											for(var j = 0; j < value.length; j++) {
												var attributes = value[j].attributes;
												var geom = value[j].geometry;
												addPointToMap(geom, attributes, "poly");
											}
										}
									}
								});
		...
		if (gravesite_id != "Select GRAVESITE")
		{
			...
			$.ajax({
				...
				success: function(result) {
					...
					//console.log("editor", objectid_gravesite_poly, gravesite_status)
					if (status == "True"){
	...
	function getUserInfo(userName) {
		...
		$.ajax({
			...
			success: function(result) {
				...
				//console.log("userInfo_array_1", userInfo_array);
	...
	function checkGraveSite(gravesite_id) {
		...
		$.ajax({
			...
			error: function(e){
				console.log("error: checkGraveSite");
				return false
			}
		});
	}
	...
	function populateSiteList(){
		...
		siteList_editor = [];
		siteList_mapbook = [];
		//console.log("userInfo_array_2", userInfo_array);
				...
				if (status == "OK"){
					...
					if (!allUser_status){
						...
						if (userInfo_array.length > 0){
							...
							{
								var userinfo = userInfo_array[i];
								var site_id_tmp = userinfo["site_id"];
								for (var n = 0; n < siteList.length; n++)
								{
									var siteItem = siteList[n];
									var site_id = siteItem["site_id"];
									var name = siteItem["name"];
									var site = name+" ("+site_id+")";
									
									if (site.indexOf(site_id_tmp) >= 0){
										siteList_user.push(site);
										console.log("siteList_user", site);
										break;
									}
								}
							}
						...
						for (var i = 0; i < userInfo_array.length; i++)
						{
							...
							if (userinfo["editor"] == 1){
								...
								for (var n = 0; n < siteList.length; n++)
								{
									...
									if (site.indexOf(site_id_tmp) >= 0){
										if (siteList_editor.contains(site))
										{
										}
										else{
											siteList_editor.push(site);
											console.log("siteList_editor", site);
										}
										break;
									}
								}
							}
							if (userinfo["mapbook"] == 1){
								var site_id_tmp = userinfo["site_id"];
								for (var n = 0; n < siteList.length; n++)
								{
									var siteItem = siteList[n];
									...
									if (site.indexOf(site_id_tmp) >= 0){
										if (siteList_mapbook.contains(site))
										{
										}
										else{
											siteList_mapbook.push(site);
											console.log("siteList_mapbook", site);
										}
										break;
									}
								}
					...
					siteList_editor.sort();
					siteList_mapbook.sort();
					...
					// editor
					for (var i = 0; i < siteList_editor.length; i++)
					{
						var site = siteList_editor[i];
						//
					...
					// mapbook
					for (var i = 0; i < siteList_mapbook.length; i++)
					{
						var site = siteList_mapbook[i];
						//
	...
	function addPointToMap(geom_json, values, geomType){
		if (geomType == "point"){
			var point = new Point(geom_json);
			point.spatialReference = map.spatialReference;
		...
		$.each(values, function(fieldName, fieldValue){
			...
			//console.log(fieldNameStr, arrayIndex);
		...
		for (var i = 0; i < siteList_editor.length; i++)
		{
			var siteItem = siteList_editor[i];
			if (siteItem.indexOf(site_id_current) >= 0){
				status = true;
				break;
			}
		}

		var all_data = table_data;
		if (status){
			...
			all_data = editor_data + table_data;
		}
		
		document.getElementById("identifyResultDiv").innerHTML = all_data;
